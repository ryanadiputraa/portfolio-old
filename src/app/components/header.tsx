	...
	return (
		<header className="flex justify-between items-center bg-white/50 dark:bg-black/50 backdrop-blur-sm px-6 sm:px-12 py-6 fixed top-0 w-full z-50">
			<Link className="flex items-center gap-2" href={'/'}>
				<Image src="/img/favicon.png" width={20} height={20} alt="" />
			...
			<nav className="hidden sm:inline-block">
                ...
                ${isDark ? '' : 'translate-x-6'}`}
							/>
							<BsSunFill className={`text-white transition-opacity ${isDark ? 'opacity-0' : ''}`} />
							<BsMoonStarsFill className={`text-black transition-opacity ${isDark ? '' : 'opacity-0'}`} />
						</div>
					</li>
				</ul>
			</nav>
